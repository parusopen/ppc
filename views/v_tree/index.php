...
function getTreeRelationItems(id){
	...
	$.post("/ppc/web/index.php?r=v_tree", {"relation_item":id},function(data){
		var j = 0, itemsHtml = '<table class="table table-striped table-bordered"><thead><tr><th></th><th>Мнемокод</th><th>Наименование</th><th style="text-align:center;"><a href="javascript:itemActions(1,1)" title="Добавить" aria-label="Добавить"><span class="glyphicon glyphicon-plus"></span></a></th></tr></thead><tbody id="RelationItemsTable">';	
		if(data!=''){
			console.log(data);	
			$.each(data, function (){	
			itemsHtml+= '<tr data-id="'+data[j]['nrn']+'"><td><input onclick="getTreeRelationItemsParams(this)" type="checkbox" name="selection[]" value="'+data[j]['nrn']+'"></td><td>'+data[j]['sdoccode']+'</td><td>'+data[j]['sdocname']+'</td><td style="text-align:center;vertical-align:middle;"><a title="Опции" aria-label="Опции" href="javascript:showActions(1, '+data[j]['nrn']+');"><span class="glyphicon glyphicon-menu-hamburger"></span></a></td></tr>';
			relation[data[j]['nrn']+'_id'] = data[j]['nrn'];
			relation[data[j]['nrn']+'_nc'] = data[j]['ncompany']; 
			relation[data[j]['nrn']+'_nv'] = data[j]['nversion'];
			relation[data[j]['nrn']+'_sc'] = data[j]['sclassif_code'];	
			relation[data[j]['nrn']+'_dc'] = data[j]['sdoccode']; 
			relation[data[j]['nrn']+'_dn'] = data[j]['sdocname'];			
			j++;
			});	
		}else{
...
function getTreeRelationItemsParams(obj){
	...
	if(!$(obj).prop('checked')){
		...
		$.post("/ppc/web/index.php?r=v_tree", {"relation_item_params":id},function(data){		
			if(data!=''){
				console.log(data);	
				$.each(data, function (){	
				...
				relation_params[data[j]['nrn']+'_nb'] = data[j]['nbase'];
				relation_params[data[j]['nrn']+'_nsc'] = data[j]['nsconf'];
				relation_params[data[j]['nrn']+'_nsd'] = data[j]['nsdoc'];
				relation_params[data[j]['nrn']+'_sut'] = data[j]['sunitcode'];
				relation_params[data[j]['nrn']+'_sun'] = data[j]['sunitname'];
				j++;
				});	
			}else{
...
function modifyItem(){
	$.post("/ppc/web/index.php?r=v_tree", $('#actionForm').serialize(), function(data){
