...
class V_tree
{
    public static function getTree($pid = 0)
    {
		...
		if($pid == 0){
			$sql = "select * from v_acatalog where signs=0 and haschildren > 0 and unitcode in ('DOCTYPES') order by rn asc";
			$ch_li = '';
		}elseif((int)$pid){
    ...
    {
		$items = \Yii::$app->db->createCommand('select * from v_doctypes where ncrn = '.$id)->queryAll();
		return json_encode($items);
	}

	public static function getTreeRelationItemsParams($id)
    {
		$items = \Yii::$app->db->createCommand('select * from v_docparams where nprn = '.$id)->queryAll();	
		return json_encode($items);
	}
